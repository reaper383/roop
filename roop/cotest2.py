...
def parse_args() -> None:
    ...
    program.add_argument('--keep-fps', help='keep the target fps', dest='keep_fps', action='store_true')
...
def create_temp(target_path: str) -> None:
    if target_path:
        temp_directory_path = os.path.join('/kaggle/working', os.path.splitext(os.path.basename(target_path))[0])
        Path(temp_directory_path).mkdir(parents=True, exist_ok=True)
        roop.globals.temp_directory_path = temp_directory_path
    else:
        roop.globals.temp_directory_path = None
